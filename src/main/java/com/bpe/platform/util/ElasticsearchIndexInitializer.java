    ...
    public void initializeIndex() {
        ...
        try {
            ...
            if (!exists) {
                ...
                // 기존 데이터를 최신 FAQ로 업데이트
                logger.info("FAQ 데이터를 최신 버전으로 업데이트합니다...");
                addSampleFAQData();
            }
        } catch (Exception e) {
    ...
    private void addSampleFAQData() throws IOException {
        ...
        List<SampleFAQ> sampleFAQs = Arrays.asList(
            ...
            new SampleFAQ(
                "9",
                "시스템 챗봇은 어떻게 사용하나요?",
                "시스템 챗봇은 Elasticsearch와 AI 모델을 결합한 RAG 방식으로 작동합니다. 채팅창에 질문을 입력하면 FAQ 문서를 검색하고, 필요시 AI가 자연스러운 답변을 생성합니다. 채팅 비우기 버튼으로 대화 내역을 초기화할 수 있으며, 사용자의 프로필 이미지가 메시지에 자동으로 표시됩니다.",
                Arrays.asList("챗봇", "시스템", "채팅", "질문", "답변", "사용법", "방법"),
                "사용법"
            ),
